...
public class EconomyCost extends Cost
{
    ...
    public EconomyCost(double quantity)
    {
        ...
        if (getEconomy() == null)
        {
            ...
            DHValidate.isTrue(vault.isEnabled(),
                    "Economy costs not available: Vault not enabled");
            int ver = PluginVersionChecker
                    .getRelease(vault.getDescription().getVersion());
            ...
            DHValidate.notNull(economyProvider,
                    "Economy costs not available: no suitable Economy plugin detected");
    ...
    {
        return vaultLegacyMode ? getEconomy().has(player.getName(), getQuantity())
    ...
    public void apply(Player player)
    {
        ...
        if (getQuantity() < 0.0)
        {
            resp = vaultLegacyMode ? getEconomy().depositPlayer(player.getName(),
                    -getQuantity()) : getEconomy().depositPlayer(player, -getQuantity());
        }
        else
        {
            resp = vaultLegacyMode ? getEconomy().withdrawPlayer(player.getName(),
                    getQuantity()) : getEconomy().withdrawPlayer(player, getQuantity());
        }
