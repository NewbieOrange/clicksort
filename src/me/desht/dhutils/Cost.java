...
public class Cost
{
     ...
     *
     * @param id
     *            ID of the item to charge, 0 for economy credits
     */
    public Cost(int id)
    {
        this(id, null, 1);
     ...
     *            ID of the item to charge, 0 for economy credits
     * @param data
     *            Data value of the item, may be null
     * @param quantity
     *            Quantity to charge, may be negative
     */
     ...
     *
     * @param costSpec
     *            The specification, in the format <i>id[:data],quantity</i>
     * @throws IllegalArgumentException
     *             if the specification contains an error
     */
    public Cost(String costSpec)
    {
        ...
        boolean durability = itemAndQuantity.length > 2
                && (itemAndQuantity[2].startsWith("d") || itemAndQuantity[2]
                        .startsWith("D"));
            ...
            else
            {
                ...
                if (pt == null)
                {
                    throw new IllegalArgumentException(
                            "cost: unknown material or potion type '" + itemType + "'");
                }
        ...
        switch (type)
        {
            ...
            case ITEM:
                String desc = (int) quantity
                        + " "
                        + Material.getMaterial(id).toString().toLowerCase()
                                .replace("_", " ");
                if (data != null)
                    desc += ":" + data;
                return desc;
    ...
    public boolean isAffordable(CommandSender sender)
    {
        ...
        {
            case MONEY:
                return economy == null || !economy.isEnabled() ? false : economy.has(
                        player.getName(), getQuantity());
            case ITEM:
                matchingInvSlots = player.getInventory().all(
                        Material.getMaterial(getId()));
                int remainingCheck = (int) getQuantity();
                for (Entry<Integer, ? extends ItemStack> entry : matchingInvSlots
                        .entrySet())
                {
                    if (getData() == null
                            || (entry.getValue().getData() != null && entry.getValue()
        ...
        switch (getType())
        {
            case MONEY:
                if (economy != null && economy.isEnabled())
                {
                    ...
                    {
                        throw new DHUtilsException("Economy problem: "
                                + resp.errorMessage);
                    }
                }
                else
                {
                    throw new DHUtilsException(
                            "Economy problem: attempt to make economy charge, but no economy plugin available");
                }
                break;
            case ITEM:
                if (getQuantity() > 0)
            ...
            case EXPERIENCE:
                ...
                break;
            case FOOD:
                player.setFoodLevel(getNewQuantity(player.getFoodLevel(), getQuantity(),
                        1, 20));
                break;
            case HEALTH:
                // note: player.getHealth() returns a double in CB 1.6+ but we
                // still work with ints for now
                player.setHealth(getNewQuantity((int) player.getHealth(), getQuantity(),
                        1, 20));
                break;
            case DURABILITY:
            ...
            case POTION_EFFECT:
                ...
                if (getQuantity() > 0)
                {
                    ...
                    player.addPotionEffect(new PotionEffect(pt, (int) getQuantity() * 20,
                            amp));
                }
                break;
        }
    ...
    public void grantItems(Player player)
    {
        ...
        if (dropped > 0)
        {
            MiscUtil.statusMessage(player,
                    "&6Your inventory is full.  Some items dropped.");
        }
    }
    
    /**
     * Take items from a player's inventory. Doesn't check to see if there is
     * enough - use playerCanAfford() for that.
     *
     * @param player
     */
    public void chargeItems(Player player)
    {
        ...
        HashMap<Integer, ? extends ItemStack> matchingInvSlots = player.getInventory()
                .all(Material.getMaterial(getId()));
        
        int remainingCheck = (int) getQuantity();
        for (Entry<Integer, ? extends ItemStack> entry : matchingInvSlots.entrySet())
        {
            if (getData() == null
                    || (entry.getValue().getData() != null && entry.getValue().getData()
                            .getData() == getData()))
            {
                remainingCheck -= entry.getValue().getAmount();
                if (remainingCheck < 0)
    ...
    {
        return data == null ? new ItemStack(getId(), quantity) : new ItemStack(getId(),
                quantity, getData());
    }
    
    private int addItems(Player player, int quantity)
    {
        Map<Integer, ItemStack> toDrop = player.getInventory().addItem(
                makeStack(quantity));
        if (toDrop.size() == 0)
        {
            return 0;
