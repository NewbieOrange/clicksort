...
/**
 * Use ProtocolLib to add a glow to unenchanted items. With thanks to Comphenix;
 * see https://gist.github.com/aadnk/4580551
 */
public class ItemGlow
{
    ...
    {
        PacketAdapter adapter = new PacketAdapter(plugin, ListenerPriority.HIGH,
    ...
    public static void setGlowing(ItemStack stack, boolean glowing)
    {
        if (!inited)
        {
            throw new IllegalStateException(
                    "ItemGlow system has not been initialised.  Call ItemGlow.init(plugin) first.");
        }
        Enchantment flag = getFlag(stack);
        if (glowing)
        {
            // if the item already has a real enchantment, let's not overwrite
            // it!
